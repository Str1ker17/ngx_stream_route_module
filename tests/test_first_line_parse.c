...
extern ngx_stream_route_type_t stream_route_first_line_parse(const u_char *buf, size_t n);
...
typedef struct {
    ...
    const u_char *buf;
    size_t len;
    ngx_stream_route_type_t expect;
} test_case_t;

static const char *decision_name(ngx_stream_route_type_t d) {
    switch (d) {
        case NGX_STREAM_ROUTE_TYPE_NOT_DECIDED:   return "NOT_DECIDED";
        case NGX_STREAM_ROUTE_TYPE_HTTP:          return "HTTP";
        case NGX_STREAM_ROUTE_PROXY_CONNECT:      return "PROXY_CONNECT";
        case NGX_STREAM_ROUTE_PROXY_PLAIN:        return "PROXY_PLAIN";
        case NGX_STREAM_ROUTE_TYPE_UNKNOWN:       return "DEFAULT";
    }
    return "?";
}
...
static int run_one(const test_case_t *tc, int index) {
    ngx_stream_route_type_t got = stream_route_first_line_parse(tc->buf, (ssize_t)tc->len);
    if (got != tc->expect) {
        fprintf(stderr, "\nFAIL #%d: %s\n", index, tc->name);
...
static test_case_t tc(const char *name, const char *s, size_t len, ngx_stream_route_type_t expect) {
    test_case_t t;
    t.name = name;
...
/* for binary buffers */
static test_case_t tcb(const char *name, const u_char *b, size_t len, ngx_stream_route_type_t expect) {
    test_case_t t;
    t.name = name;
    t.buf = b;
    t.len = len;
...
int main(void) {
    ...
    test_case_t tests[] = {
        ...
        tc("len0 empty", "", 0, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("len1 'G'", "GET /x HTTP/1.1\r\n", 1, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("len2 'GE'", "GET /x HTTP/1.1\r\n", 2, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("len3 'GET' no space", "GET /x HTTP/1.1\r\n", 3, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("len1 single space should be DEFAULT (invalid, no need to read more)", " ", 1, NGX_STREAM_ROUTE_TYPE_UNKNOWN),
        tcb("bin spaces-only line should be DEFAULT", bin_spaces_only, sizeof(bin_spaces_only), NGX_STREAM_ROUTE_TYPE_UNKNOWN),

        /* ===== PRI / h2c preface ===== */
        tc("PRI full preface -> HTTP", "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n", strlen("PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n"), NGX_STREAM_ROUTE_TYPE_HTTP),
        tc("PRI partial 'PRI ' -> NOT_DECIDED", "PRI * HTTP/2.0\r\n", 4, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("PRI 'PRI *' -> HTTP", "PRI * HTTP/2.0\r\n", 5, NGX_STREAM_ROUTE_TYPE_HTTP),
        ...
        tc("CONNECT partial 'CONNE' -> NOT_DECIDED", "CONNECT x", 5, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("CONNECT partial 'CONNECT' (no space) -> NOT_DECIDED", "CONNECT", strlen("CONNECT"), NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        ...
        tc("partial 'GET https:' -> NOT_DECIDED", "GET https://host/x HTTP/1.1\r\n", 10, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("partial 'GET https:/' -> NOT_DECIDED", "GET https://host/x HTTP/1.1\r\n", 11, NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        ...
        tcb("binary tab between GET and / -> DEFAULT", bin_no_space_with_lf, sizeof(bin_no_space_with_lf), NGX_STREAM_ROUTE_TYPE_UNKNOWN),
        ...
        /* ===== binary / odd bytes ===== */
        tcb("binary garbage with space then / -> HTTP (router sees /)", bin_garbage, sizeof(bin_garbage), NGX_STREAM_ROUTE_TYPE_HTTP),
        tcb("binary NUL in method then space / -> DEFAULT (invalid, but should not be NOT_DECIDED forever)", bin_nul_in_method, sizeof(bin_nul_in_method), NGX_STREAM_ROUTE_TYPE_HTTP),
        ...
        tc("not decided: only first space found but no target", "POST ", strlen("POST "), NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
        tc("not decided: only first space found but no target 2", "X ", strlen("X "), NGX_STREAM_ROUTE_TYPE_NOT_DECIDED),
